(function () {
  ...
  let u_time_location;
  let u_fractal_color;
  let u_screen_size, u_roudness, u_invVP;

  let FRAME_W = 800.0, FRAME_H = 800.0;

  const viewMatrix = create$1();
  const projMatrix = create$1();
  const invVP = create$1();
  ...
  function UpdateMatrices() {
      ...
      const eye = fromValues(x, y, z);
      const at = fromValues(0, 0, 0);
      const up = fromValues(0, 1, 0);
      lookAt(viewMatrix, eye, at, up);

      const aspect = FRAME_W / FRAME_H;
      perspective(projMatrix, Math.PI / 3, aspect, 0.1, 10.0);

      const vp = create$1();
      multiply(vp, projMatrix, viewMatrix);
      invert(invVP, vp);
  }
